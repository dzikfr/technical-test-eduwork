...
const createUser = async (req, res) => {
  ...
  //WRITEFILE
  const { us_id, us_name, us_password, us_email, us_phone_number, us_address } =
    req.body;
...
const updateUser = async (req, res) => {
  try {
    ...
    const userIndex = usersData.findIndex(
      (user) => user.us_id === updatedUser.us_id
    );
...
const loginUser = async (req, res) => {
  try {
    ...
    const { _id, us_id, us_name, us_email, us_phone_number, us_address } = user;

    return res
      .status(200)
      .send({
        message: "ok",
        data: { _id, us_id, us_name, us_email, us_phone_number, us_address },
      });
  } catch (error) {
    return res.status(500).send({ message: error.message });
  }
